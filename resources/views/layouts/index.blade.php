...
<head>
    <meta charset="utf-8" />
    <title>{{ trim($__env->yieldContent('title')) ?: getPageTitle() }} | Repalogic Sakola</title>
...
<body id="kt_body" class="{{ $bodyClass }}">
    ...
    <script>
        // Pusatkan menu aktif di area aside dengan scroll halus setelah layout siap.
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var activeItem = menu.querySelector('.menu-item-active, .menu-item-here');
            if (!activeItem) {
                return;
            }
            ...
            var scrollToCenter = function() {
                ...
                var itemTopInMenu = itemRect.top - menuRect.top + menu.scrollTop;
                var targetTop = Math.max(0, itemTopInMenu - (menu.clientHeight / 2) + (itemRect.height / 2));
            ...
            // Tunggu inisialisasi KT menu dan pembukaan submenu aktif.
            setTimeout(function() {
                window.requestAnimationFrame(scrollToCenter);
            }, 250);
        });
    </script>
    <!--end::Page Scripts-->
