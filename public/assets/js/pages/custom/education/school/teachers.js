	...
	var _demo = function() {
		...
		for (var i = 1; i <= 60; i++) {
			demoRows.push({
				RecordID: i,
				OrderID: 'TCH-' + (1000 + i),
				CompanyName: 'Department ' + ((i % 8) + 1),
				CompanyAgent: 'Teacher ' + i,
				CompanyEmail: 'teacher' + i + '@example.com',
				Country: 'Country ' + ((i % 8) + 1),
				ShipCountry: ['US', 'ID', 'SG', 'AU', 'JP', 'DE', 'CA', 'BR'][i % 8],
				ShipDate: ('0' + ((i % 12) + 1)).slice(-2) + '/' + ('0' + ((i % 28) + 1)).slice(-2) + '/2025',
				IssueDate: ('0' + ((i % 12) + 1)).slice(-2) + '/' + ('0' + ((i % 28) + 1)).slice(-2) + '/2025',
				Status: ((i % 4) + 1),
				Type: ((i % 3) + 1),
				Gender: i % 2 === 0 ? 'M' : 'F'
			});
